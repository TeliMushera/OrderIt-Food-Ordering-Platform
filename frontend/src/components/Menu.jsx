...
const Menu = () => {
  ...
  const menuCategories = menus
    ? menus.flatMap((menuDoc) =>
        (menuDoc.menu || []).map((category) => ({
          menuId: menuDoc._id,
  ...
  const submitMenuCreation = async (e) => {
    ...
    if (result && result.meta && result.meta.requestStatus === "fulfilled") {
      ...
      alert(result.payload || "Unable to create menu");
    }
  };
      ...
      {loading ? (
        ...
        menuCategories.map((menuCategory) => {
          const deleteMenu = async () => {
            ...
            try {
              await api.delete(`/v1/eats/stores/${id}/menus/${menuCategory.menuId}`, {
                withCredentials: true,
              });
              dispatch(getMenus(id));
            } catch (err) {
          ...
          return (
            <div key={`${menuCategory.menuId}-${menuCategory.category}`}>
              <div className="d-flex align-items-center">
                <h2 className="mr-2">{menuCategory.category}</h2>
              ...
              {menuCategory.items && menuCategory.items.length > 0 ? (
                <div className="row">
                  {menuCategory.items.map((fooditem) => (
                    <Fooditem
                      key={fooditem._id}
                      fooditem={fooditem}
                      restaurant={id}
            ...
            <form
              onSubmit={async (e) => {
                ...
                if (created && created._id) {
                  const targetMenuId = itemToAdd.menuId || menuId;
                  dispatch(
                    addItemToMenu({
                      menuId: targetMenuId,
              ...
              <div className="form-group">
                ...
                >
                  ...
                  {menuCategories.map((category) => (
                    <option key={`${category.menuId}-${category.category}`} value={category.category}>
                      {category.category}
                    </option>
                  ))}
                </select>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      console.error(err);
                      alert(err.response?.data?.message || err.message || "AI generation failed");
                    }
                  }}
                >
                  AI desc
